...
for i in range(10):
    ...
    with torch.no_grad():
        for X, y, _ in test_loader:
            X, y = X.to(device), y.to(device)
            pred = model(X)
            cm += torch.nn.functional.one_hot(pred.argmax(1),2).T @ torch.nn.functional

    print(f"Test Loss: {test_loss:.6f}, Test Acc: {test_acc:.6f}")
    wandb.log({"test_loss": test_loss, "test_acc": test_acc, "confusion":cm, "healty":healty, "epileptic":epileptic})
    wandb.finish()
