...
import random
from pyhmeasure.pyhmeasure import PyHmeasure as h_score
...
for run_n in range(N_RUNS):    
    ...
    wandb.init(project='emc_nested_LOSO-v2' , name=f'HScore_weighted_run_{run_n}', reinit=True)
    ...
    for ss in range(len(labels)):
        ...
        for feature_name in feature_names:
            ...
            for montage, segment_length, significance in itertools.product(montages, segment_lengths, p_values):
                ...
                c0_scores=y_prob[y_val==0]
                c1_scores=y_prob[y_val==1]
                hmeas=h_score(2,2,c0_scores, c1_scores).h

                if hmeas > best_hmeas:
                    print(f'New best H-measure for {feature_name}: {bac80}')
                    best_auc = auc
                    best_model = model
                    ...
                    best_bac80 = bac80
                    best_hmeas = hmeas
                    best_classifiers[feature_name] = (best_auc, best_model, best_val_data, significant_feats, test_data, best_bac80, hmeas)    
        ...
        for feature_name in best_classifiers:
            ...
            wandb.log({f'aucs/{feature_name}': best_classifiers[feature_name][0],                     
                       f'bac80/{feature_name}': best_classifiers[feature_name][5],                     
                       f'h-measure/{feature_name}':best_classifiers[feature_name][6]}, step=ss)
        ...
        print(f'Individual probability scores from each model: {X_test_lr}')
    ...
    c0_scores=y_probs_outer[y_true_outer==0]
    c1_scores=y_probs_outer[y_true_outer==1]
    hmeas=h_score(2,2,c0_scores, c1_scores).h
    ...
    wandb.log({'BAC': final_bac,
                ...
                'H-Measure': final_hmeas})

    if not os.path.exists('/space/gzanardini/emc/'):
        os.mkdir('/space/gzanardini/emc/')

    run_summary.to_csv(f'/space/gzanardini/emc/HMEAS_weighted_run_{run_n}_seed_{seed}.csv', index=False)    
    prediction_summary.to_csv(f'/space/gzanardini/emc/HMEAS_weighted_run_{run_n}_predictions_seed_{seed}.csv', index=False)
