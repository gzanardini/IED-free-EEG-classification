...
SPLIT_RATIO=0.3
RUN_NAME='BAC'
PROJECT_NAME='emc_LOSO_subjects_v2'
...
def train_classifier(feature_name, montage, segment_length, significance, train_idxs, val_idxs, test_idxs, y_train, y_val, seed):
    ...
    return feature_name, auc, bac80, model, data[val_idxs][:, significant_feats], significant_feats, test_data
...
for run_n in range(N_RUNS):    
    ...
    for ss, subject in enumerate(unique_subjects):
        ...
        for feature_name in feature_names:
            best_classifiers[feature_name] = None
            with ThreadPoolExecutor(max_workers=32, thread_name_prefix='xgb_multi') as executor:
                futures = []
                for montage, segment_length, significance in itertools.product(montages, segment_lengths, p_values):
                    futures.append(executor.submit(train_classifier, feature_name, montage, segment_length, significance, train_idxs, val_idxs, test_idxs, y_train, y_val, seed))

                for future in futures:
                    result = future.result()
                    if result is not None:
                        feature_name, auc, bac80, model, val_data, significant_feats, test_data = result
                        if best_classifiers[feature_name] is None or auc > best_classifiers[feature_name][0]:
                            print(f'New best BAC for {feature_name}: {bac80}')
                            best_classifiers[feature_name] = (auc, model, val_data, significant_feats, test_data, bac80)
