...
def main():
    ...
    for montage, feature_name, segment_length, combiner in itertools.product(montages, feature_names, segment_lengths, combiners):
        ...
        for run_n in range(N_RUNS):
            RUN_NAME=f'{feature_name}_{montage}_{segment_length}s_{combiner}run_{run_n}'
            
            # if not checkpoint:
            #     if RUN_NAME == 'mst_CAR_2s_kurtrun_0' :    #   for length 5 if crashes -> 'cc+cwt+utm+gcc+gplv_run_3'
            #         checkpoint = True
