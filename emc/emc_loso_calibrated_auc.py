...
for run_n in range(N_RUNS):    
    ...
    wandb.init(project='emc_LOSO_calibrated' , name=f'AUCweighted_run_{run_n}', reinit=True)
    ...
    for ss in range(len(labels)):
        ...
        for feature_name in feature_names:
            ...
            for montage, segment_length, significance in itertools.product(montages, segment_lengths, p_values):
                ...
                if auc > best_auc:
                    print(f'New best AUC for {feature_name}: {bac80}')
                    best_auc = auc
                    best_model = model
    ...
    run_summary.to_csv(f'/space/gzanardini/emc/auc_weighted_run_{run_n}_seed_{seed}.csv', index=False)    
    prediction_summary.to_csv(f'/space/gzanardini/emc/auc_weighted_run_{run_n}_predictions_seed_{seed}.csv', index=False)
