...
FEAT_FOLDER = '/space/gzanardini/tuh_epoched/split'
...
def save_results(run_summary, prediction_summary, subject_summary, run_n, seed):
    ...
    run_summary.to_csv(f'{LOG_FOLDER}{RUN_NAME}_{run_n}_seed_{seed}.csv', index=False)
    prediction_summary.to_csv(f'{LOG_FOLDER}{RUN_NAME}_{run_n}_predictions_seed_{seed}.csv', index=False)
    subject_summary.to_csv(f'{LOG_FOLDER}{RUN_NAME}_{run_n}_subject_predictions_seed_{seed}.csv', index=False)
