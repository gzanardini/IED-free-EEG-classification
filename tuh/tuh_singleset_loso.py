...
for montage, feature_name, segment_length, significance in itertools.product(montages, feature_names, segment_lengths, p_values):
    ...
    for run_n in range(N_RUNS):   
        ...
        wandb.init(project=PROJECT_NAME, name=f'{feature_name}_{montage}_{segment_length}s_p<{significance}_run_{run_n}', reinit=True)
        ...
        for ss, subject in enumerate(unique_subjects):
            ...
            ps=np.array([mannwhitneyu_test(features[train_idxs][y_train==0][:,col], features[train_idxs][y_train==1][:,col]) for col in range(features.shape[1])])
            ...
            model.fit(cp.array(features[train_idxs][:,significant_feats]), cp.array(labels[train_idxs]))

            print('Training data shape:', features[train_idxs][:,significant_feats].shape)
            print('Test data shape:', features[test_idxs][:,significant_feats].shape)

            y_pred=model.predict(cp.array(features[test_idxs][:,significant_feats]))
            y_score=model.predict_proba(cp.array(features[test_idxs][:,significant_feats]))[:,1]
