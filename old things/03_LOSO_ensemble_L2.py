...
for run_n in range(N_RUNS):    
    ...
    wandb.init(project='LOSO ensemble' , name=f'run_{run_n}_no_w_opt_scorepreds_l2', reinit=True)
    ...
    for ss, subject in enumerate(unique_subjects):
        ...
        subject_recap = {feature_name: [] for feature_name in feature_names}
        ...
        for feature_name in feature_names:
            ...
            for montage, segment_length, significance in itertools.product(montages, segment_lengths, p_values):
                ...
                if auc > best_auc:
                    print(f'New best AUC for {feature_name}: {auc}')
                    best_auc = auc
                    best_model = model
                    best_val_data = data[val_idxs][:, significant_feats]
                    test_data=data[test_idxs][:, significant_feats]
                    best_bac80 = bac80
                    best_classifiers[feature_name] = (best_auc, best_model, best_val_data, significant_feats, test_data)    
        ...
        for feature_name in best_classifiers:
            ...
            subject_recap[feature_name]= best_classifiers[feature_name][0]

        X_train_lr = np.array(X_train_lr).T
        X_test_lr = np.array(X_test_lr).T

        # fit logistic regression model
        lr = LogisticRegression(penalty='l2', fit_intercept=False, solver='lbfgs')
        ...
        w_tilde0 = np.random.randn(X_train_lr.shape[1])
        result = minimize(logistic_loss, w_tilde0, args=(X_train_lr, y_val, lr), method='L-BFGS-B') # Optimize weights
        w_opt = softmax_weights(result.x)
        lr.coef_ = w_opt.reshape(1, -1)
        #lr.intercept_ = np.array([0.0])

        print('Optimized weights:', w_opt)
        print('Optimized intercept:', lr.intercept_)
        '''

        wandb.log({f'weights': lr.coef_ },step=ss)
        ...
        wandb.log(subject_recap, step=ss)
