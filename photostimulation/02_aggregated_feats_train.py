...
car_feats=np.load('data/feats/aggregated_features_CAR.npy')
cz_feats=np.load('data/feats/aggregated_features_Cz.npy')
bipolar_feats=np.load('data/feats/aggregated_features_BipolarDB.npy')
laplacian_feats=np.load('data/feats/aggregated_features_Laplacian.npy')
...
for i,(feats, name) in enumerate(zip(feats_list, feats_names)):
    ...
    for significance in significance_threhsholds:
        ...
        for j in range(N_RUNS):
            ...
            wandb.init(project=f'xgboost_stats_aggre' , name=f'{name}_p<{significance}_run_{j}', reinit=True)
            ...
            for i,subject in enumerate(unique_subjects):
                ...
                model=XGBClassifier(scale_pos_weight=ratio, n_jobs=4, seed=seed, device='cuda:3')
